...
describe('MatDialog', () => {
  ...
  it('should set the proper animation states', () => {
    let dialogRef = dialog.open(PizzaMsg, { viewContainerRef: testViewContainerRef });
    let dialogContainer: MsDialogContainer =
        viewContainerFixture.debugElement.query(By.directive(MsDialogContainer))!
        .componentInstance;
...
describe('MatDialog with animations enabled', () => {
  ...
  it('should return the current state of the dialog', fakeAsync(() => {
    ...
    expect(dialogRef.getState()).toBe(MsDialogState.OPEN);
    dialogRef.close();
    viewContainerFixture.detectChanges();

    expect(dialogRef.getState()).toBe(MsDialogState.CLOSING);
    ...
    flush();
    expect(dialogRef.getState()).toBe(MsDialogState.CLOSED);
  }));
});
...
class ComponentWithTemplateRef {
  localValue: string;
  dialogRef: MsDialogRef<any>;

  @ViewChild(TemplateRef) templateRef: TemplateRef<any>;

  setDialogRef(dialogRef: MsDialogRef<any>): string {
...
class PizzaMsg {
  constructor(public dialogRef: MsDialogRef<PizzaMsg>,
